...
interface IntermediateChromeProps {
  onContinue: () => void
  contentWidth?: number
  children: React.ReactNode
}

export default function IntermediateChrome({ onContinue, contentWidth = 330, children }: IntermediateChromeProps) {
  const router = useRouter()

  return (
    <div style={{ background: '#fefefe', minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      {/* Header */}
      <div style={{ position: 'relative', paddingTop: '20px', height: '50px', marginBottom: '13px', flexShrink: 0, display: 'flex', alignItems: 'flex-start', justifyContent: 'center' }}>
        <button
          onClick={() => router.back()}
          style={{ position: 'absolute', top: '20px', left: '20px', background: 'none', border: 'none', cursor: 'pointer', padding: '4px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
        >
          <BackArrow />
        </button>
        <NebulaLogo />
      ...
      {/* Content */}
      <div style={{ width: `${contentWidth}px`, margin: '0 auto', flex: 1, paddingTop: '12px' }}>
        {children}
      </div>

      {/* Footer */}
      <div style={{ width: `${contentWidth}px`, margin: '0 auto', paddingBottom: '24px', paddingTop: '20px', flexShrink: 0 }}>
        <button
          onClick={onContinue}
          style={{
            display: 'block',
